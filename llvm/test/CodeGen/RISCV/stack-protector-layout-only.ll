; RUN: llc -mtriple=riscv64 -stop-after=prolog-epilog -o - %s | FileCheck %s
; RUN: llc -mtriple=riscv64 -O0 -global-isel -stop-after=prolog-epilog -o - %s | FileCheck %s
...
; With ssp only the large array moves; the rest keep their order.
...
; Nothing to lay out, so stackProtectorLayoutOnly stays false.
...
; Control: without a stack protector attribute the objects keep their order.
